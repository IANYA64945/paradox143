...
function stopRain() {
  ...
  function startAmbientEvent(
    ...
    paradoxStats.inc(`weather_${type}`);
...
function startPebble(
  ...
  function renderClimatePanel(){
    ...
    for(const type of types){
      ...
      if(type!=='normal' && activeEvent===type) row.classList.add('active');
      ...
      if(unlocked){
        row.addEventListener('click',()=>{
          ...
          if(busyScene()){
            showSpecialHint('Espera a terminar la escena actual ♡');
            return;
          }
          ...
          if(activeEvent){
            ...
            clearTimeout(eventEndingTimer);
            endAmbientEvent(old);
            setTimeout(()=>startAmbientEvent(type),650);
          }else{
            startAmbientEvent(type);
          }
        });
      }
  ...
  climateBtn.addEventListener('click',e=>{
    ...
    e.stopPropagation();
    renderClimatePanel();
    climatePanel.classList.toggle('show');
  });
