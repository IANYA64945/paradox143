...
function startTravelingStar(
  ...
  function finishNearMoon(){ if(done) return; done=true; const rect=star.getBoundingClientRect(); starBurst(rect.left+rect.width/2,rect.top+rect.height/2,18); star.classList.add('moonReached'); moon.classList.add('idleMoonKiss'); const mr=moon.getBoundingClientRect(); createMoonKiss(mr.left+mr.width/2,mr.top+mr.height/2); showIdleMessage('🌙 La estrella alcanzó a la luna y dejó un pequeño brillo a su alrededor.',3700); setTimeout(()=>{moon.classList.remove('idleMoonKiss'); star.remove();},650); finishIdle('traveler',19000); }
