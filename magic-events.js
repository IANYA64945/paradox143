...
function startPebble(
  ...
  /* =====================================================
     BOTÓN DE CLIMAS DESBLOQUEABLES
  ===================================================== */

  const CLIMATE_LETTERS={
    stars:'weather-stars',
    fog:'weather-fog',
    rain:'weather-rain',
    snow:'weather-snow',
  ...
  function climateLetters(){
    try{
      const raw=localStorage.getItem('paradox143_letters_v1');
  ...
  climateBtn.textContent='☁';
  climateBtn.setAttribute('aria-label','Cambiar clima');
  document.body.appendChild(climateBtn);

  const climatePanel=document.createElement('div');
  climatePanel.id='climatePanel';
  climatePanel.innerHTML=`
    <div class="climatePanelHeader">
      <span>CLIMA ☁</span>
      <button id="climatePanelClose" type="button">×</button>
    </div>
    <div id="climatePanelList"></div>
  `;
  document.body.appendChild(climatePanel);

  const climatePanelList=climatePanel.querySelector('#climatePanelList');

  function refreshClimateUnlock(){
    const letters=climateLetters();
    const any=Object.values(CLIMATE_LETTERS).some(id=>letters.has(id));
    climateBtn.classList.toggle('visible',any);
  }

  function renderClimatePanel(){
    const letters=climateLetters();
    const types=['normal','stars','fog','rain','snow','storm'];
    climatePanelList.innerHTML='';

    for(const type of types){
      const unlocked=type==='normal' || letters.has(CLIMATE_LETTERS[type]);
      const row=document.createElement('button');
      row.type='button';
      row.className='climateChoice';
      if(!unlocked){ row.classList.add('locked'); row.disabled=true; }
      const weatherState=
        window.ParadoxWeather &&
        window.ParadoxWeather.getState
        ? window.ParadoxWeather.getState()
        : {
            type:null,
            manual:false
      ...
      if(type==='normal'){
        row.innerHTML='<span>☾</span><strong>Campo normal</strong><small>siempre disponible</small>';
      }else{
        const [mark,name]=CLIMATE_NAMES[type];
        row.innerHTML=`<span>${unlocked?mark:'🔒'}</span><strong>${unlocked?name:'???'}</strong><small>${unlocked?'usar este clima':'aún no descubierto'}</small>`;
      }
        ...
        row.addEventListener(
          ...
          ()=>{
            ...
            if(
              ...
              climateBtn.classList.remove(
                'climateDenied'
              );

              void climateBtn.offsetWidth;

              climateBtn.classList.add(
                'climateDenied'
              );
            ...
            if(
              ...
              climateBtn.textContent=
                CLIMATE_NAMES[type][0];


              /*
                Volvemos a renderizar un poco después
                para marcar correctamente "ACTIVO".
              */
              setTimeout(
                renderClimatePanel,
                360
      ...
      climatePanelList.appendChild(row);
    }
  }

  climateBtn.addEventListener(
    'click',
    e=>{
      ...
      /*
        El render ya no accede a variables privadas
        de otro módulo, por lo que el menú puede
        abrirse de forma segura.
      */
      renderClimatePanel();

      climatePanel.classList.toggle(
        'show'
      );
    }
  );

  climatePanel.querySelector('#climatePanelClose').addEventListener('click',()=>climatePanel.classList.remove('show'));

  document.addEventListener('pointerdown',e=>{
    if(!climatePanel.classList.contains('show')) return;
    if(climatePanel.contains(e.target) || climateBtn.contains(e.target)) return;
    climatePanel.classList.remove('show');
  },true);

  setInterval(refreshClimateUnlock,1800);
  refreshClimateUnlock();
